...
if [ "$APP_NAME" = "risc_s" ] ; then
    ...
    yosys -p "synth_ecp5 -json out.json" -D $BOARD $OPTIONS uart_tx.sv uart_rx.sv decoder.sv regfile.sv utils.sv exec.sv divider.sv multiplier.sv flash_master.sv $RAM_FILE io.sv timer.sv csr.sv mem_space.sv ecp5pll.sv risc_s.sv
    nextpnr-ecp5 --package CABGA381 --speed $SPEED --85k --freq 62.50 --json out.json --lpf $LPF_FILE --textcfg out.config
    ecppack --db ../prjtrellis-db out.config out.bit
    openFPGALoader -b ulx3s out.bit
else
if [ "$APP_NAME" = "risc_p" ] ; then
    ...
    yosys -p "synth_ecp5 -json out.json" -D $BOARD $OPTIONS uart_tx.sv uart_rx.sv decoder.sv regfile.sv utils.sv exec.sv divider.sv multiplier.sv flash_master.sv $RAM_FILE io.sv timer.sv csr.sv mem_space.sv ecp5pll.sv risc_p.sv
    nextpnr-ecp5 --package CABGA381 --speed $SPEED --85k --freq 62.50 --json out.json --lpf $LPF_FILE --textcfg out.config
    ecppack --db ../prjtrellis-db out.config out.bit
    openFPGALoader -b ulx3s out.bit
fi
